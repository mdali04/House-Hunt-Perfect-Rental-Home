...
router.post('/register',upload.single('profile'),validate(Register),async(req,res)=>{
...
// update the profile
router.put('/updateprofile/:id',verifyToken,upload.single('profile'),async(req,res)=>{
    const { name,address,email,phone,password } = req.body;
    const profile = req.file?.filename;
    try {
        const customer = await Customer.findById(req.params.id);
...
router.get('/viewhouse',async(req,res)=>{
    try{
...
router.get('/housesss',async (req, res) => {
    try {
      const { city, startprice, endprice, bedrooms, facilities } = req.query;
...
router.get('/viewhousedetails/:id',async(req,res)=>{
    try {
...
router.post('/bookhouse/:id',verifyToken,async(req,res)=>{
    try{
...
router.get('/viewbooking/:id',verifyToken,async(req,res)=>{
    try {
        const id = req.params.id;
