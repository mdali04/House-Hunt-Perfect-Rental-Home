...
router.post("/register",upload.single('profile'),validate(Register),async(req,res,next)=>{
    try{
...
const verifyToken = (req,res,next)=>{
    ...
    if(token){
        Jwt.verify(token,process.env.JWT_SECRET || "VDFJBIBGK2132345KJFWF432R4",(err,decoded)=>{
            if(err){
                return res.status(400).json({
                    success:false,
...
// update the profile
router.put("/updateprofile/:id",verifyToken,upload.single('profile'),async(req,res,next)=>{
    try{
        const rental = await Rental.findById(req.params.id);
        if(rental){
            const {name,address,email,phone,password} = req.body;
...
// change password
router.put("/changepassword/:id",verifyToken,async(req,res,next)=>{
    try{
        const rental = await Rental.findById(req.params.id);
        if(rental){
...
router.post("/addhouse/:id",verifyToken,upload1.array('images',5),validate(AddHouse),async(req,res,next)=>{
    try{
...
// get all houses
router.get('/houses/:id',verifyToken,async(req,res,next)=>{
    try{
...
// get a single house
router.get('/housedetail/:id',verifyToken,async(req,res,next)=>{
    try{
...
// update a house
router.put('/house/:id',verifyToken,upload1.array('images',5),async(req,res,next)=>{
    try{
        const apartment = await Apartment.findById(req.params.id);
        if(apartment){
            const {title,price,location,type,facilities,description,bedrooms,bathrooms,squareFootage,availableFrom,parkingAvailable,petPolicy,heating,cooling,deposit,leaseTerm,nearbySchools,publicTransport,shoppingCenters,groceryStores,security,contact,website} = req.body;
...
// delete a house
router.delete('/deletehouse/:id',verifyToken,async(req,res,next)=>{
    try{
        const apartment = await Apartment.findById(req.params.id);
        if(apartment){
...
// view booking
router.get('/booking/:id',verifyToken,async(req,res,next)=>{
    try {
        const id = req.params.id ;
...
// dashboard
router.get('/dashboard/:id', verifyToken, async (req, res, next) => {
    try {
        const id = req.params.id ;
